...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/home/scholz_la/Desktop/untitled_lastrun.py',
...
looming = visual.ShapeStim(
    win=win, name='looming',
    size=[1.0, 1.0], vertices='circle',
    ori=0.0, pos=(-0.5, 0), anchor='center',
    lineWidth=0.0,     colorSpace='rgb',  lineColor=[0.0000, 0.0000, 0.0000], fillColor='black',
...
for thisTrial_2 in trials_2:
    ...
    while continueRoutine and routineTimer.getTime() < 30.0:
        ...
        if noise.status == STARTED:
            ...
            if tThisFlipGlobal > noise.tStartRefresh + 30-frameTolerance:
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        # *looming* updates
        if looming.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            looming.frameNStart = frameN  # exact frame index
            looming.tStart = t  # local t and not account for scr refresh
            looming.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(looming, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'looming.started')
            looming.setAutoDraw(True)
        if looming.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > looming.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
                looming.tStop = t  # not accounting for scr refresh
                looming.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'looming.stopped')
                looming.setAutoDraw(False)
        if looming.status == STARTED:  # only update if drawing
            looming.setSize((0.1*t, 0.1*t), log=False)
